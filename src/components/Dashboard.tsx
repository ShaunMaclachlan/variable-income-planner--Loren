...
import { formatPence, hourlyRatePenceForDate } from '../domain/money'
...
export function Dashboard({ state, rules, onPlan, onCalendar }: Props) {
  ...
  const differenceLabel = assessment.differencePence >= 0 ? `${formatPence(assessment.differencePence)} forecast buffer` : `${formatPence(Math.abs(assessment.differencePence))} remaining`
  ...
  return <>
    ...
    <section className="card hero-card">
      ...
      <h2>{assessment.differencePence >= 0 ? 'Your total forecast is above the planning target' : 'Your assessment target still needs attention'}</h2>
    ...
    <section className={`card buffer-card ${assessment.differencePence >= 0 ? 'safe' : 'short'}`}>
      <div className="section-heading"><div><span className="eyebrow">Room to change</span><h2>{assessment.differencePence >= 0 ? 'Your forecast cushion' : 'Hours still needed'}</h2></div></div>
      <div className="buffer-number"><strong>{Math.abs(assessment.bufferHours).toFixed(2)}h</strong><span>at the base rate of {formatPence(hourlyRatePenceForDate(rules, state.settings.assessmentEnd))}/hour</span></div>
      {assessment.differencePence >= 0
    ...
    <section className="card">
      ...
      <div className="metric-pair"><div><strong>{formatPence(assessment.workedGrossPence)}</strong><span>Earned / confirmed</span></div><div><strong>{formatPence(assessment.plannedGrossPence)}</strong><span>Planned</span></div></div>
      <div className="metric-pair"><div><strong>{formatPence(assessment.grossPence)}</strong><span>Total forecast</span></div><div><strong>{formatPence(assessment.targetPence)}</strong><span>Planning target</span></div></div>
